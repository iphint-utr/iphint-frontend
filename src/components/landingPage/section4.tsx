  ...
  return (
    <section className="min-h-[500px] w-full bg-[#fafafa] py-20 font-sans text-slate-900">
      <div className="w-full px-6 lg:px-16 xl:px-24 flex flex-col gap-12">
        ...
        {/* Tabs */}
        <div className="flex gap-1 overflow-x-auto hide-scrollbar lg:justify-center">
          {carouselData.map((item, idx) => (
            <button
              key={item.key}
              onClick={() => goTo(idx)}
              className={`shrink-0 px-3 md:px-5 py-2 rounded-xl text-sm font-semibold whitespace-nowrap transition-colors duration-200 ${
                idx === activeIndex
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_1fr] gap-10 lg:gap-0 items-stretch">
          ...
          <div className="relative min-h-72 lg:min-h-0 rounded-2xl lg:rounded-none overflow-hidden lg:mr-10">
            <div className="absolute inset-0 bg-[radial-gradient(#e2e8f0_2px,transparent_2px)] [background-size:24px_24px] opacity-70" />
          ...
          <div className="flex flex-col justify-center gap-5">
            <span className="inline-block w-fit bg-slate-900 text-white px-4 py-1.5 rounded-full text-sm font-semibold tracking-wide">
