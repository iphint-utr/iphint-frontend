...
import { BarChart3, LayoutDashboard, LogOut, Search, ShieldCheck, Users } from 'lucide-react';
...
export default function AdminSidebar({
  ...
  return (
    <>
      ...
      <aside
        className={[
          'fixed left-0 top-0 z-40 h-screen w-72 border-r border-slate-200 bg-white shadow-sm transition-transform duration-200',
          open ? 'translate-x-0' : '-translate-x-full',
        ].join(' ')}
      >
        <div className="flex h-16 items-center justify-between border-b border-slate-200 px-6">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.32em] text-slate-500">IPHINT</p>
            <p className="mt-1 text-base font-semibold tracking-tight text-slate-950">{t('sidebar.brand')}</p>
          </div>
